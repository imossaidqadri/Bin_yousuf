        ...
        for (const project of projects) {
          ...
          if (projectStat.isDirectory()) {
            ...
            if (!mainImageCheck.exists) {
              ...
              console.log(
                `  ⚠️  Empty main-image.webp (${mainImageCheck.size} bytes)`
              );
              issues.push(mainImageCheck);
            } else {
              console.log(
                `  ✅ main-image.webp (${(mainImageCheck.size / 1024).toFixed(1)}KB)`
              );
            }

            // Check gallery images
            const projectFiles = await readdir(projectPath);
            const galleryFiles = projectFiles.filter(
              file => file.startsWith('gallery-') && file.endsWith('.webp')
            );
            ...
            if (galleryFiles.length === 0) {
              ...
              for (const galleryFile of galleryFiles) {
                ...
                if (galleryCheck.size === 0) {
                  console.log(
                    `  ❌ Empty ${galleryFile} (${galleryCheck.size} bytes)`
                  );
                  issues.push(galleryCheck);
                } else if (galleryCheck.size < 1000) {
                  console.log(
                    `  ⚠️  Small ${galleryFile} (${galleryCheck.size} bytes)`
                  );
                } else {
                  console.log(
                    `  ✅ ${galleryFile} (${(galleryCheck.size / 1024).toFixed(1)}KB)`
                  );
                }
              }
            }
